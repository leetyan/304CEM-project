...
module.exports = {
    ...
    item: function (req, callback) {
        sql = mysql.format('SELECT * FROM comment WHERE username = ?', [req.params.id]);
        return connection.query(sql, callback);
    },
    add: function (req, callback) {
        sql = mysql.format('INSERT INTO comment SET ?', req.body);
        return connection.query(sql, callback);
    },
    delete: function (req, callback) {
        sql = mysql.format('DELETE FROM comment WHERE username = ?', [req.params.id]);
        return connection.query(sql, callback);
    },
    ...
    patch: function (req, callback) {       
        sql = mysql.format('UPDATE comment SET ? WHERE username = ?', [req.body, req.params.id]);
        return connection.query(sql, callback);
    }
};